...
<script>
  export default {
    ...
    methods: {
      ...
      changeMultilingualism(){

        // Убираю выбор всех языков
        for(let elem in this.$store.state.multilingualism){
          this.$store.state.multilingualism[elem] = false;
        }

        // Устанавливаю текущей выбранный язык
        this.$store.state.multilingualism[this.lang] = true;
        console.log(this.$store.state.multilingualism)
      },
      //! Выход из аккаунта
     async logout(){
